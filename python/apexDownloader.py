...
keys = ['t','temp','pres','rh'] #Datetime, Temperature, Pressure, Relative Humidity. Need to find units.
...
for year in range(2006,2017):
    for month in range(1,13):
        ...
        for i in keys:
            wx[i] = []
        tempData = [[],[]] #holds data for the entire month in case files are saved on wrong dates.
        presData = [[],[]]
        rhData = [[],[]]
        ...
        for day in range(1,calendar.monthrange(year,month)[1]+1):
            ...
            f = urllib2.urlopen("http://www.apex-telescope.org/weather/Historical_weather/temperature/%s_temperature.log.gz"%datestr)
            ...
            f = urllib2.urlopen("http://www.apex-telescope.org/weather/Historical_weather/pressure/%s_pressure.log.gz"%datestr)
            compressedFile = StringIO.StringIO()            compressedFile.write(f.read())
            compressedFile.seek(0)
            decompressedFile = gzip.GzipFile(fileobj = compressedFile, mode = 'r')
            for line in decompressedFile.readlines():
            ...
            f = urllib2.urlopen("http://www.apex-telescope.org/weather/Historical_weather/humidity/%s_humidity.log.gz"%datestr)
            compressedFile = StringIO.StringIO()            compressedFile.write(f.read())
            compressedFile.seek(0)
            decompressedFile = gzip.GzipFile(fileobj = compressedFile, mode = 'r')
            for line in decompressedFile.readlines():
                if len(line.split(" ")) == 2:
                    (t,rh) = line.split()
                    rhData[0] += [datetime.datetime.fromtimestamp(float(t)+18000.)] #18000 seconds added to adjust for GMT time
                    rhData[1] += [float(rh)]
        ...
        print wxArray[0]
        with open("/n/home10/anwang16/merra2_work/merra2_products/wx_data/APEX_wx_%s-%s.txt"%(year,month),'wb') as f:
