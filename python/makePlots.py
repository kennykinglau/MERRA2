...
if __name__ == '__main__':
    ...
    parser.add_option(
        "-s", dest="start", type=str, help="-s option: Year to produce a plot for."
    )
    ##TODO: Detect when the daterange goes out of the range of MERRA2, and switch to using GFS.

    parser.add_option("-e", dest="end", type=str, help="-e option: year end.")
    ...
    for year in range(int(tstart), int(tend) + 1):
        ...
        # Chajnantor
        for i in glob.glob(
            "/n/home10/anwang16/merra2_work/merra2_products/%s/final/%s_%s%s*_gndData%s_%s*"
            % (currentSite, currentSite, year, month, gndData, cloud)
        ):
            # for i in files:
        ...
        dtype = [
            ...
            ('day', str),
            ('hour', str),
            ('BK100 Tsky (Krj)', float),
            ('BK150 Tsky (Krj)', float),
            ('BK220 Tsky (Krj)', float),
            ('BK270 Tsky (Krj)', float),
            ('PWV (um)', float),
        ]
        Txtdata = np.array(
            [
        ...
        np.savetxt(
            "%s_BK_MERRA2.txt" % year,
            Txtdata,
            fmt="%.4i %.2i %.2i %.2i %3.3f %3.3f %3.3f %3.3f %3.3f",
            header="Year, Month, Day, Hour (UTC), BK100 Tsky (K_rj), BK150 Tsky (K_rj), BK220 Tsky (K_rj), BK270 Tsky (K_rj), PWV (um)",
        )
        ...
        plot(
            data['t_tipper'],
            data['tipper850_interp'],
            'r.-',
            label='Resampled MERRA2 Predictions',
        )
        plot(
            data['t'],
            data['tipper850'],
            'g.',
            label='Original 3-hour MERRA2 Predictions',
        )
        legend()
        grid()
        ylabel("Tsky (K_rj)")
        xlabel("Date")
        title(
            "Zenith Brightness Temperature (Tsky): %s %s%s" % (currentSite, year, month)
        )
        savefig(
            "merra2_products/plots/%s_timeseries_resampled_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        # show()
        close()
        ...
        general = (tipperTsky > 0.0) & (merraTsky > 0.0)
        tipperTsky = tipperTsky[general]
        merraTsky = merraTsky[general]
        fit = np.polyfit(merraTsky, tipperTsky, 1)
        unsaturated = (tipperTsky < 240.0) & (
            merraTsky < 240.0
        )  # filter out saturated data (completely opaque atmosphere)
        ...
        plot(
            ...
            fit_fn(limits),
            color='red',
            label="best fit for all data (R = %.2f): y = "
            % (np.corrcoef(merraTsky, tipperTsky)[1][0])
            + str(fit_fn)[2:],
        )
        plot(
            limits,
            bestfit(limits),
            color='green',
            label="best fit for Tsky < 240K (R = %.2f): y = "
            % (np.corrcoef(merraTsky[unsaturated], tipperTsky[unsaturated])[1][0])
            + str(bestfit)[2:],
        )
        ...
        grid()
        title(
            "Comparison of Zenith Brightness Temperature (Tsky) at %s for %s%s"
            % (currentSite, year, month)
        )
        print(year, np.corrcoef(merraTsky, tipperTsky)[1][0], len(tipperTsky))
        savefig(
            "merra2_products/plots/%s_correlation_resampled_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        ...
        scatter(
            data['t'][new],
            data['averaged_Tsky_tipper'][new],
            color='red',
            label='850GHz Tipper Data',
        )
        ...
        savefig(
            "merra2_products/plots/%s_timeseries_averaged_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        ...
        if len(tipperTsky) == 0:
            continue
        fit = np.polyfit(merraTsky, tipperTsky, 1)
        unsaturated = (data['averaged_Tsky_tipper'] < 240.0) & (
            data['tipper850'] < 240.0
        )  # filter out saturated data (completely opaque atmosphere)
        unsaturatedFit = np.polyfit(
            data['tipper850'][unsaturated], data['averaged_Tsky_tipper'][unsaturated], 1
        )
        fit_fn = np.poly1d(fit)
        bestfit = np.poly1d(unsaturatedFit)
        fit_perfect = np.poly1d([1, 0])
        ...
        plot(
            ...
            % (
                np.corrcoef(
                    data['tipper850'][unsaturated],
                    data['averaged_Tsky_tipper'][unsaturated],
                )[1][0]
            )
            + str(bestfit)[2:],
        )
        ...
        print(year, np.corrcoef(merraTsky, tipperTsky)[1][0], len(tipperTsky))
        savefig(
            "merra2_products/plots/%s_correlation_averaged_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        # savefig("plots/tsky/Comparison%s%s%s"%(currentSite,year,month))
        # show()
        ...
        errorbar(
            data['t'][new],
            residual,
            yerr=data['Tsky_sigma'][new],
            label='Averaged, mean-subtracted residual (%s points): ' % (len(residual))
            + r'$\chi^2=%s$' % chisquared,
        )
        ...
        legend()
        title(
            "Residual of Tsky (MERRA > Tipper) at %s for %s%s"
            % (currentSite, year, month)
        )
        savefig(
            "merra2_products/plots/%s_residual_averaged_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        ...
        errorbar(
            data['t_tipper'],
            residual,
            yerr=data['tipper_unc'],
            label='Resampled, mean-subtracted residual (%s points): ' % (len(residual))
            + r'$\chi^2=%s$' % chisquared,
        )
        grid()
        legend()
        ...
        xlabel("Date")
        title(
            "Mean-Subtracted Residual of Tsky (MERRA > Tipper) at %s for %s%s"
            % (currentSite, year, month)
        )
        savefig(
            "merra2_products/plots/%s_residual_resampled_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        # show()
        close()
        ...
        savefig(
            "merra2_products/plots/%s_fractional_averaged_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        ...
        xlabel("Date")
        title(
            "Fractional Residual of Tsky (MERRA > Tipper) at %s for %s%s"
            % (currentSite, year, month)
        )
        savefig(
            "merra2_products/plots/%s_fractional_resampled_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        # show()
        close()
        ...
        tauFilter = data['tau'] > 0.0
        scatter(
            data['t_tipper'][tauFilter],
            data['tau'][tauFilter],
            color='red',
            label='Tipper opacity',
        )
        scatter(
            data['t'], data['tau850'], color='blue', label='MERRA2-predicted opacity'
        )
        legend()
        grid()
        title("Optical Depth (Tau) at %s for %s%s" % (currentSite, year, month))
        savefig(
            "merra2_products/plots/%s_tau_averaged_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        # show()
        clf()
        ...
        generalFilter = tau_m < 8.0  # get rid of clear outliers
        smallTauFilter = (tau_m < 4.0) & (
            data['tau850'] < 4.0
        )  # only do small tau. The fit should be better in this case, so it refers to bestfit variable.
        ...
        plot(
            ...
            color='red',
            label="best fit for all data (R = %.2f): y = "
            % (np.corrcoef(allTau, tau_m[generalFilter])[1][0])
            + str(taufit)[2:],
        )
        plot(
            allTau,
            BestTauFit(allTau),
            color='green',
            label="best fit for tau < 4 (R = %.2f): y = "
            % (np.corrcoef(smallTau, tau_m[smallTauFilter])[1][0])
            + str(BestTauFit)[2:],
        )
        ...
        savefig(
            "merra2_products/plots/%s_taucorrelation_averaged_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        # show()
        clf()
        ...
        grid()
        title(
            "Residual of tau (MERRA > Tipper) at %s for %s%s"
            % (currentSite, year, month)
        )
        savefig(
            "merra2_products/plots/%s_tauresidual_averaged_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        # savefig("plots/tsky/Residual%s%s%s"%(currentSite,year,month))
        # show()
        ...
        fig = figure(figsize=(25, 10))
        fractionalresidual = (
            2 * (allTau - tau_m[generalFilter]) / (allTau + tau_m[generalFilter])
        )
        ...
        savefig(
            "merra2_products/plots/%s_taufractional_averaged_%s_groundData%s_%s"
            % (siteInitial, year, gndData, cloud)
        )
        # savefig("plots/tsky/FracResidual%s%s%s"%(currentSite,year,month))
        # show()
