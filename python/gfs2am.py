...
BADVAL = -99999.0  # placeholder data that is missing or not defined on level
BADVAL_TEST = -99998.0
...
H2O_SUPERCOOL_LIMIT = 238.0  # ice assumed below this temperature [K]
PASCAL_ON_MBAR = 100.0  # conversion from mbar (hPa) to Pa
...
# of GFS.
H2O_TOP_PLEVEL = 29.0  # Ignore H2O above this pressure level.
...
parser.add_argument("lat", help="site latitude [deg], (-90 to 90)", type=float)
parser.add_argument("lon", help="site longitude [deg], (-180 to 180)", type=float)
parser.add_argument("altitude", help="site altitude [m]", type=float)
parser.add_argument("gfsdate", help="GFS production date (YYYYMMDD)", type=str)
parser.add_argument("gfscycle", help="GFS production cycle (0, 6, 12, 18)", type=int)
parser.add_argument("gfsprod", help="GFS product: anl or f000 - f384", type=str)
...
if args.altitude < -500.0:
    parser.error("invalid altitude")
if len(args.gfsdate) != 8:
    parser.error("invalid GFS production date format")
try:
    gfsdatetime = datetime.datetime.strptime(args.gfsdate, "%Y%m%d")
except:
    parser.error("bad GFS production date format")
...
request_url += PRODUCT_REQUEST_FORMAT.format(
    args.gfscycle, LATLON_GRID_STR, args.gfsprod
)
for lev in LEVELS:
...
request_url += SUBREGION_REQUEST_FORMAT.format(leftlon, rightlon, toplat, bottomlat)
request_url += CYCLE_REQUEST_FORMAT.format(args.gfsdate, args.gfscycle)
...
for i, lev in enumerate(LEVELS):
    ...
    try:
        x = grid_interp(
            grbindx.select(name="Geopotential Height", level=lev)[0].values, u, v
        )
        z.append(x)
    except:
        z.append(BADVAL)
    try:
        x = grid_interp(grbindx.select(name="Temperature", level=lev)[0].values, u, v)
        T.append(x)
    except:
        T.append(BADVAL)
    try:
        x = grid_interp(
            grbindx.select(name="Ozone mixing ratio", level=lev)[0].values, u, v
        )
    ...
    try:
        x = grid_interp(
            grbindx.select(name="Relative humidity", level=lev)[0].values, u, v
        )
    ...
    try:
        x = grid_interp(
            grbindx.select(name="Cloud mixing ratio", level=lev)[0].values, u, v
        )
        cloud_mr.append(x)
    except:
        cloud_mr.append(0.0)
...
print(
    LAYER_HEADER.format(
        args.gfsdate, args.gfscycle, product_str, args.lat, args.lon, args.altitude
    )
)
for i, lev in enumerate(LEVELS):
