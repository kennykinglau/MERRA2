...
if __name__ == '__main__':
    ...
    for year in range(int(tstart), int(tend) + 1):
        filename = glob.glob(
            "/n/home10/anwang16/merra2_work/merra2_products/%s/final/%s_%s*_gndData%s_%s*"
            % (currentSite, currentSite, year, gndData, cloud)
        )[0]
        ...
        residualTsky = (
            data['Tsky_tipper'] / transmission - data['tipper850_interp']
        )  # Tsky residual. window transmission must be accounted for
        ...
        intervals = [90.0, 30.0, 10.0, 1.0, 1.0 / 4]
        # intervals = [90.,30.,10.,1.,1.0/4,1.0/24,1.0/144] #interval sizes in number of days

        for interval in intervals:
            numIntervals = int(360.0 / interval)
            dateList = [
                tstart + datetime.timedelta(days=i * interval)
                for i in range(numIntervals + 1)
            ]  # creates a list of separating dates.
            intervalAverages = (
                []
            )  # for each specific interval: over the year, over 3 months, over 1 day, etc.
            if type(data['t_tipper'][0]) != datetime.datetime:
                continue
            for i in range(numIntervals):
                intervalFilter = (data['t_tipper'] > dateList[i]) & (
                    data['t_tipper'] < dateList[i + 1]
                )
                residuals = residualTsky[
                    intervalFilter
                ]  # only get the residuals in the interval.
                intervalAverages += [mean(residuals)]
                residualTsky[intervalFilter] = residualTsky[intervalFilter] - mean(
                    residuals
                )
        ...
        with open(
            "/n/home10/anwang16/merra2_work/merra2_products/analysis/TimescaleAnalysis_%s_%s_gndData%s_%s.csv"
            % (currentSite, year, gndData, cloud),
            "w",
        ) as f:
