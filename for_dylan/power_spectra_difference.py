...
parser.add_argument('-fbi2d','--file_base_instantaneous_2d')
parser.add_argument('-fei2d','--file_end_instantaneous_2d')
parser.add_argument('-fbt2d','--file_base_time_averaged_2d')
parser.add_argument('-fet2d','--file_end_time_averaged_2d')

parser.add_argument('-fbi3d','--file_base_instantaneous_3d')
parser.add_argument('-fei3d','--file_end_instantaneous_3d')
parser.add_argument('-fbt3d','--file_base_time_averaged_3d')
parser.add_argument('-fet3d','--file_end_time_averaged_3d')
...
FILE_BASE_INST_2D = args.file_base_instantaneous_2d
FILE_END_INST_2D = args.file_end_instantaneous_2d
FILE_BASE_TAVG_2D = args.file_base_time_averaged_2d
FILE_END_TAVG_2D = args.file_end_time_averaged_2d

FILE_BASE_INST_3D = args.file_base_instantaneous_3d
FILE_END_INST_3D = args.file_end_instantaneous_3d
FILE_BASE_TAVG_3D = args.file_base_time_averaged_3d
FILE_END_TAVG_3D = args.file_end_time_averaged_3d
...
setsOfData_tavg_3d = {}
for date in dates:
    setsOfData_inst_2d[date] = nc.Dataset(FILE_BASE_INST_2D+date+FILE_END_INST_2D)
    setsOfData_tavg_2d[date] = nc.Dataset(FILE_BASE_TAVG_2D+date+FILE_END_TAVG_2D)
    setsOfData_inst_3d[date] = nc.Dataset(FILE_BASE_INST_3D+date+FILE_END_INST_3D)
    setsOfData_tavg_3d[date] = nc.Dataset(FILE_BASE_TAVG_3D+date+FILE_END_TAVG_3D)

message = "Loaded "+str(len(setsOfData_tavg_3d))+ " days of data."
if PRINT_MESS: print(message)
messages.append(message)

#Calculate the Cartesian coordinates of the MERRA2 grid points
data_merra_2d = setsOfData_inst_2d[dates[0]][VAR_2D][0]
...
for date in dates:
    data_merra_inst_2d = setsOfData_inst_2d[date][VAR_2D]
    data_merra_tavg_2d = setsOfData_tavg_2d[date][VAR_2D]
    data_merra_inst_3d = setsOfData_inst_3d[date][VAR_3D]
    data_merra_tavg_3d = setsOfData_tavg_3d[date][VAR_3D]

    #Now because there are 24 times in the 2d data but only 8 in the 3d, some of the 2d data is eliminated or averaged
    data_merra_inst_2d = data_merra_inst_2d[::3,:,:]
    #TODO: There ought to be a more efficient way to do this with numpy arrays, but I can't figure it out
    averaged_array = np.zeros((8,data_merra_tavg_2d.shape[1],data_merra_tavg_2d.shape[2]))
    for j in range(8):
        averaged_array[j] = (data_merra_tavg_2d[3*j]+data_merra_tavg_2d[3*j+1]+data_merra_tavg_2d[3*j+2])/3
    data_merra_tavg_2d = averaged_array
    
    for t in range(8):
        
        #Interpolate 3D variable at South Pole to have even spacing for FFT
        SP_array = data_merra_tavg_3d[t,:,0,0] - data_merra_inst_3d[t,:,0,0]
        ...
        array_2d = data_merra_tavg_2d[t] - data_merra_inst_2d[t]
        ...
        if MAKE_IMAGES:
            imageName = "%s,%s%s,%s.%s.t=%i.png" % (FILE_BASE_INST_3D,FILE_BASE_INST_2D,VAR_3D,VAR_2D,date,t)
            ...
            pressTitle = "Delta %s %s vs. Pressure" % (VAR_3D,DIM_DICT[VAR_3D])
            ...
            freqTitle = "Power Spectrum of Delta %s" % VAR_3D
            ...
            twodgraphTitle = "Delta %s %s" % (VAR_2D,DIM_DICT[VAR_2D])
            ...
            twod_freqs_title = "Power Spectrum of Delta %s" % VAR_2D
...
#Make test graphics
imageName = "%s,%s%s,%s.test.png" % (FILE_BASE_INST_3D,FILE_BASE_INST_2D,VAR_3D,VAR_2D)
message = "Making image %s..." % imageName
if PRINT_MESS: print(message)
messages.append(message)
fig, ((press_graph,oned_power),(twod_graph,twod_power)) = plt.subplots(2,2)
...
txtfile = open(os.path.join(folderPath,'0%s,%s.%s,%s.%s-%s.results.txt' % (FILE_BASE_INST_3D,FILE_BASE_TAVG_2D,VAR_3D,VAR_2D,START_DATE.strftime('%Y%m%d'),END_DATE.strftime('%Y%m%d'))),'w')
txtfile.write('\n'.join(messages))
txtfile.close()