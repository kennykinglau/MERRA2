...
SPT_time_indices = (SPT_time_numbers // 3).astype(int)

SPT_for_correlation = np.ma.array(np.zeros(8*len(dates)))
...
description = 'log_10(%s Filtered %s Variance)' % (FREQ_FILTER,VAR) if TAKE_LOG else '%s Filtered %s Variance' % (FREQ_FILTER,VAR)
...
integrals_for_correlation = integrals[~invalidValues]
SPT_for_correlation = SPT_for_correlation[~invalidValues]

if TAKE_LOG:
    integrals_for_correlation = np.log10(integrals_for_correlation)
    invalidValues = np.isnan(integrals_for_correlation) | np.isinf(integrals_for_correlation)
    SPT_for_correlation = SPT_for_correlation[~invalidValues]
    integrals_for_correlation = integrals_for_correlation[~invalidValues]

regression = sts.linregress(integrals_for_correlation,SPT_for_correlation)
...
fig, ax = plt.subplots()
ax.plot(integrals_for_correlation,SPT_for_correlation,'bo',label='Data points')
ax.plot(integrals_for_correlation,intercept + slope*integrals_for_correlation,'r-',label = 'Best fit')
...
imageName = '0%s.%s,%s.%s.%s-%s.scatter%s.png' % (FREQ_FILTER,FILE_BASE_INST,FILE_BASE_TAVG,VAR,START_DATE.strftime('%Y%m%d'),END_DATE.strftime('%Y%m%d'),modeString)
imagePath = os.path.join(folderPath,imageName)
fig.savefig(imagePath)
plt.close(fig)
...
if TAKE_LOG:
    integrals = np.log10(integrals)
    invalidValues = np.isnan(integrals) | np.isinf(integrals)
    times = times[~invalidValues]
    integrals = integrals[~invalidValues]

fig, ax1 = plt.subplots()
times = 3*np.arange(integrals.shape[0])
varianceLabel = description
ratioLabel = ratio
ax1.plot(times,integrals,'b-',label=varianceLabel)
ax1.plot([],[],'r-',label=ratioLabel)
ax1.set_xlabel('Time [hrs]')
ax1.set_ylabel(varianceLabel)
ax1.set_title("%s and %s vs. Time" % (description,ratio))
ax1.legend()
...
imageName = '0%s.%s,%s.%s.%s-%s.time_series%s.png' % (FREQ_FILTER,FILE_BASE_INST,FILE_BASE_TAVG,VAR,START_DATE.strftime('%Y%m%d'),END_DATE.strftime('%Y%m%d'),modeString)
imagePath = os.path.join(folderPath,imageName)
fig.savefig(imagePath)
plt.close(fig)

txtfile = open(os.path.join(folderPath,'0%s.%s,%s.%s.%s-%s.results%s.txt' % (FREQ_FILTER,FILE_BASE_INST,FILE_BASE_TAVG,VAR,START_DATE.strftime('%Y%m%d'),END_DATE.strftime('%Y%m%d'),modeString)),'w')
txtfile.write('\n'.join(messages))
txtfile.close()