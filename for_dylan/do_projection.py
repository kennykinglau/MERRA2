...
def reproject(data):
    ...
    lat = np.linspace(-90.0, -85.0, num=data.shape[0])
    lon = np.linspace(-180.0, 180.0, num=data.shape[1])

    # Next, make the cartesian grid
    x = np.linspace(-5.0, 5.0, num=100)
    y = np.linspace(-5, 5.0, num=100)
...
if __name__ == '__main__':
    ...
    begin = time.time()
    out = reproject(np.random.normal(loc=0.0, scale=1.0, size=(50, 180)))
    end = time.time()
