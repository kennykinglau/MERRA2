...
def process_data(x_for_correlation,x_data,x_time_numbers,y_for_correlation,y_data,y_time_numbers,
    ...
    RANDOM = False, PRINT_MESS = False, TIME_STREAM = False,
    START_YEAR = 2019, START_MONTH=3,START_DAY=22,
    END_YEAR = 2019, END_MONTH = 12, END_DAY=31):
...
def make_model(xLabels,xRangeStrings,xDataTuple,yLabel,yRangeString,yData,FOLDER_NAME,
    ...
    ABSX=False,ABSY=False,LOGX=False,LOGY=False,runID=''):
    ...
    print(np.any(np.isnan(features),axis=1).shape)
    print(np.any(np.isinf(features),axis=1).shape)
    x_invalid = np.any(np.isnan(features),axis=1) | np.any(np.isinf(features),axis=1)
    print(labels.shape)
    print(labels.mask)
    print(np.isnan(labels).shape)
    print(np.isinf(labels).shape)
    y_invalid = np.isnan(labels) | np.isinf(labels) | labels.mask
    
    invalidIndices = x_invalid | y_invalid
    
    print(invalidIndices.shape)
    labels = labels[~invalidIndices]
    labels = labels.filled()
    
    #Resize invalidValues for the x-data
    temp_list = []
    for i in range(len(xDataTuple)):
        temp_list.append(invalidIndices)
    invalidIndices = np.column_stack(tuple(temp_list))
    print(invalidIndices.shape)
    features = features[~invalidIndices]
    if features.shape[0]==features.size:
        features = features.reshape((int(features.size/len(xDataTuple)),len(xDataTuple)))
    print(np.max(features),np.min(features))
    print(np.max(labels),np.min(labels))
    print(features.shape)
    print(labels.shape)
    #print(np.any(labels.mask))
    #print("Here")
    ...
    imageName = '%s%svs%s_%s_history_%s.png' % (yRangeString,yLabel,independentVariableNames,architectureString,runID)
    imagePath = os.path.join(folderPath,imageName)
    fig.savefig(imagePath)
    plt.close(fig)
    ...
    ax.set_title(graphTitle)
    imageName = '%s%svs%s_%s_test_scatter_%s.png' % (yRangeString,yLabel,independentVariableNames,architectureString,runID)
    imagePath = os.path.join(folderPath,imageName)
    fig.savefig(imagePath)
    plt.close(fig)
    ...
    if len(xDataTuple)==1:
        ...
        imageName = '%s%svs%s_%s__function_%s.png' % (yRangeString,yLabel,independentVariableNames,architectureString,runID)
        imagePath = os.path.join(folderPath,imageName)
        fig.savefig(imagePath)
        plt.close(fig)
    
    return yRangeString,yLabel,independentVariableNames,rho,r_sq
